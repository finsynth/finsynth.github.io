      ...
      for (let col = 0; col < columns + 1; col++) {
        ...
        if (row === 0) {
          if (col === 0) {
            ...
            cell.textContent = this.getColumnLetter(col - 1)
          }
        } else {
          if (col === 0) {
        ...
        cell.style.left =
          col === 0
            ? '0px'
            : `${this.cellWidth / 4 + (col - 1) * this.cellWidth + (col > middleColIndex ? heroContentWidth - this.cellWidth : 0)}px`
  ...
  createHeroBottomGrid() {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < columns + 1; col++) {
        ...
        const fadeProgress = row / (rows - 1)
        const baseOpacity = 0.3
        const minOpacity = 0.0
        const fadeOpacity = baseOpacity - fadeProgress * (baseOpacity - minOpacity)
        const backgroundColor =
          col === 0 ? `rgba(246, 248, 250, ${fadeOpacity})` : `rgba(255, 255, 255, ${fadeOpacity})`
...
export default function useExcelGrid() {
  useEffect(() => {
    const grid = new ExcelGridBackground()
    return () => grid.destroy()
  }, [])
}
