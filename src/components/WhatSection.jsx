...
/**
 * "What" — the full set of workflows FinSynth handles, as an auto-scrolling
 * column that loops on its own (a seamless vertical marquee). Rows near the top
 * of the masked window read as "handled" — black text + a ticked box — while
 * rows lower down stay grey with an empty box. Each row's box ticks on (and off)
 * as it crosses the active line, so the checklist reads as continuously
 * completing itself while it scrolls. The band stays on white.
 */
const WORKFLOWS = [
  'Building models',
  'Earnings updates',
